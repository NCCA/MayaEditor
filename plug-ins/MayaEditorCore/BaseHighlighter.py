...
class BaseHighlighter(QSyntaxHighlighter):
    ...
    def _create_format_rgb(self, style_colour: QColor, style: str = "") -> QTextCharFormat:
        fmt = QTextCharFormat()
        fmt.setForeground(QBrush(style_colour))
        if "bold" in style:
            fmt.setFontWeight(QFont.Bold)
        if "italic" in style:
            fmt.setFontItalic(True)
    ...
    def __init__(
        ...
        braces: Sequence[str],
        maya_commands_func: Callable[[], Sequence[str]],
    ) -> None:
        super().__init__(parent)
        self.styles: Dict[str, QTextCharFormat] = {
            "keyword": self._create_format_rgb(QColor(255, 166, 87)),
            "operator": self._create_format_rgb(QColor(255, 166, 87)),
            "brace": self._create_format("darkGray"),
            "defclass": self._create_format_rgb(QColor(255, 166, 87)),
            "deffunc": self._create_format_rgb(QColor(121, 192, 234)),
            "string": self._create_format_rgb(QColor(165, 214, 255)),
            "string2": self._create_format_rgb(QColor(165, 214, 255)),
            "comment": self._create_format_rgb(QColor("Gray")),
            "self": self._create_format_rgb(QColor(121, 192, 255)),
            "numbers": self._create_format("GhostWhite"),
            "maya": self._create_format("SpringGreen"),
        }
        ...
        rules = (
            [(rf"\b{w}\b", 0, self.styles["keyword"]) for w in keywords]
            + [(rf"{o}", 0, self.styles["operator"]) for o in operators]
            + [(rf"{b}", 0, self.styles["brace"]) for b in braces]
            + [
